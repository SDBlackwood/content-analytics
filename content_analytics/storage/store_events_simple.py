...
def store_events():
    ...
    try:
            ...
            for _, partition_messages in records.items():
                for message in partition_messages:
                    try:
                        ...
                        MediaEvent.model_validate_json(message.value)
        ...
        __store_as_parquet(pd.DataFrame(messages), s3_client)
    ...
    for event_type in df["event_type"].unique():
        ...
        for date, date_df in event_df.groupby("date"):
            ...
            s3_client.upload_fileobj(
                Fileobj=parquet_buffer, Bucket=settings.s3_bucket, Key=s3_key
            )
