...
I chose to implement the storage component of the system as it contained the most unique aspects to the challenges domain and allowed for experimentation in parquet which would
give me more insights into the potential ML ready operations which could be performed on the data.

Todo
- Simulates 100k records being pulled from Kafka every 10s.
    - Set up a local kafka deployment
    - Set up a script which adds records to kafka
- Runs a airflow worfklow every 10s to pull from kafka
    - Set up local airflow to run script on a cron jon
